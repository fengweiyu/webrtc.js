...
class WebRtcTalk 
{
    ...
    {
        if (mediaDataSource.type.toLowerCase() !== 'talk') 
        {
            console.log('webrtcTalk requires an webrtc MediaDataSource input!');
        }
    ...
    load() 
    {
        ...
        this.m_PeerConnection.ontrack = function(event){
			if (event.track.kind === 'audio') {
				// 处理音频流
				console.log('pcTalk audio:',event.streams.length);//, event.track.length
    ...
    play() 
    {
		navigator.mediaDevices.getUserMedia({audio: true }).then(function(stream) {//后续考虑使用同步阻塞的
			...
			oPlayer.m_PeerConnection.addTransceiver('video', { direction: 'sendrecv' });//sendonly
			oPlayer.m_PeerConnection.addTransceiver(stream.getTracks()[0], { direction: 'sendrecv' });//sendonly
